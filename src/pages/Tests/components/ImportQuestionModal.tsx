...
export const ImportQuestionModal: React.FC<ImportQuestionModalProps> = ({
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (useAI && geminiKey) {
        ...
        // Pass the image file if we are in image mode
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title="Nhập đề thông minh (AI Import)"
      width={700}
    >
        ...
        {preview.length === 0 ? (
          <>
            <div style={{ 
              ...
              <span>AI tự động bóc tách đề từ: Văn bản, Link, PDF hoặc Ảnh chụp bài tập.</span>
            </div>
            ...
            {sourceType === 'text' ? (
              ...
              <div style={{
                ...
                <Icon name={sourceType === 'pdf' ? "file-text" : "image"} size={40} style={{ color: 'var(--text-4)' }} />
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-4)' }}>
                    {selectedFile ? `${(selectedFile.size / 1024 / 1024).toFixed(2)} MB` : `Hỗ trợ ${sourceType === 'pdf' ? '.pdf' : '.jpg, .png, .webp'}`}
                  </div>
                </div>
                <input
                  type="file"
              ...
              {preview.map((q, i) => (
                <div key={i} style={{ 
                  ...
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--primary)', marginBottom: 4 }}>
                    CÂU {i + 1} ({q.type === 'mcq' ? 'Trắc nghiệm' : 'Tự luận'})
                  </div>
