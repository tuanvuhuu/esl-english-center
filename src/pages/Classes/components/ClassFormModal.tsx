...
const EMPTY: Form = {
  name: '', level: 'A1', ageGroup: '', teacherId: '', roomId: '',
  branchId: '', academicYearId: '', maxStudents: '15', feePerMonth: '',
...
export const ClassFormModal: React.FC<ClassFormModalProps> = ({ open, onClose, onSuccess, classData }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let classId: string
      if (isEdit && classData) {
        const updated = await updateClass(String(classData.id), payload)
        classId = updated.id
      } else {
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? 'Chỉnh sửa lớp học' : 'Mở lớp mới'} width={600}>
